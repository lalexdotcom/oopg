...
- **Global scope**: `/workspaces/oopg/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/workspaces/oopg/.claude/notes/`)
...
2. Glob `/workspaces/oopg/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/workspaces/oopg/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/workspaces/oopg/.claude/notes/` — works in any directory
